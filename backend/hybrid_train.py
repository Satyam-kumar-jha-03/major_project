"""
Hybrid Training System for AI vs Human Media Detector - COMPLETE VERSION
"""
import numpy as np
import pandas as pd
...
class HybridTrainer:
    ...
    def analyze_dataset(self, train_df):
        ...
        is_video_frames = any(
            'frame' in name.lower() or 'video' in name.lower()
        ...
        if is_video_frames and num_samples < VIDEO_MODE_THRESHOLD * num_classes:
            ...
            print("SELECTED: VIDEO SEQUENCE model")
    ...
    def build_model(self, num_classes, dataset_info):
        print(f"Building {self.model_type} classifier...")
    ...
    def create_dataset_csv(self):
        ...
        if not test_rooms:
            print("No test folder found. Creating 80/20 train/test split...")
            train_df, test_df = train_test_split(
                train_df, test_size=0.2, random_state=42, stratify=train_df['tag']
            )
            train_df.to_csv('train.csv', index=False)
            test_df.to_csv('test.csv', index=False)
            print(f"Created train.csv ({len(train_df)}) and test.csv ({len(test_df)})")
        else:
